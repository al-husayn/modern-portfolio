...
function useMediaQuery(q) {
  ...
  useEffect(() => {
    ...
    return () => mqRef.current.removeEventListener("change", onChange);
  }, [q]);
...
These hooks are suitable for non-sensitive preferences that improve UX. Do not store authentication credentials or tokens in Web Storage; instead keep those in server-managed HttpOnly, Secure, SameSite cookies or behind a Backend-For-Frontend (BFF) layer.
...
// Note: only store non-sensitive preferences (not auth tokens or credentials)
