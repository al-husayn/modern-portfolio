...
```tsx
// useFetch - request with loading/error state
function useFetch(url, options) {
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  // Use a normalized key instead of raw options object to avoid unnecessary
  // effect re-runs caused by object reference changes
...
**Important:** To prevent unnecessary refetches when passing object-based options, provide a stable reference using `useMemo`:
```tsx
const options = useMemo(() => ({ method: 'GET' }), []);
useFetch(url, options);
```
This ensures the serialized `optionsKey` remains stable across renders unless the options actually change.
### `useInfiniteScroll`
...
function useMediaQuery(q) {
  ...
  const [matches, setMatches] = useState(() => {
    ...
    mqRef.current = window.matchMedia(q);
    return mqRef.current.matches;
  });

  useEffect(() => {
    if (typeof window === "undefined") return;
    // create and keep a single MediaQueryList for this effect execution
...
function useOnlineStatus() {
  // server-safe default, read navigator inside effect
  ...
  useEffect(() => {
    ...
    // initialize state in case it changed before effect ran
...
These hooks are suitable for non-sensitive preferences that improve UX. Store things like theme, language, and draft state in browser storage, and keep authentication credentials on the server using HttpOnly, Secure, SameSite cookies or a Backend-For-Frontend (BFF) pattern.
...
// Note: store only non-sensitive preferences.
...
function useClipboard() {
  ...
  const copy = async (text) => {
    ...
    if (!window.isSecureContext || !navigator.clipboard) {
      const err = 'Clipboard API unavailable (not secure context)';
      setError(err);
      return false;
    }
